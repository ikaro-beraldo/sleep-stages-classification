classdef Sleep_archictecture_interface < matlab.apps.AppBase
    ...
    properties (Access = public)
        ...
        CalculateLatencyCheckBox        matlab.ui.control.CheckBox
    ...
    properties (Access = private)
        ...
        Latency_result % Stores the latency analysis results
    end
...
%                     x_ticks = round(duration_bouts_UI.XTick * (length(app.All_Sort) * app.rdStepSelecttheEpochLengthSecEditField.Value / 3600),1);
    ...
    methods (Access = private)
        ...
        function RefreshButtonPushed(app, event)
            ...
            switch app.Selected_Radio_Button
                case 'whole'        % Call the function to execute the architecture analysis without any segmentation
                    ...
                    if app.CalculateLatencyCheckBox.Value   % Only calculate latency if this option is enabled
                        app.Latency_result = app_latency(app.All_Sort,app.rdStepSelecttheEpochLengthSecEditField.Value,0);
                    end
                case 'segmented'    % Uses the value informed by the user to execute the function
                    ...
                    if app.CalculateLatencyCheckBox.Value   % Only calculate latency if this option is enabled
                        app.Latency_result = app_latency(app.All_Sort,app.rdStepSelecttheEpochLengthSecEditField.Value,app.SegmentLengthHoursEditField.Value);
                    end
                case 'custom'       % Uses the custom intervals informed by the user
                    ...
                    if app.CalculateLatencyCheckBox.Value   % Only calculate latency if this option is enabled
                        app.Latency_result = app_latency(app.All_Sort,app.rdStepSelecttheEpochLengthSecEditField.Value,segment_length);
                    end
            end
        ...
        function SaveButtonPushed(app, event)
            ...
            % Latency
            if app.CalculateLatencyCheckBox.Value   % Only calculate latency if this option is enabled
                Latency_NREM = app.Latency_result.first_nrem_minutes;
                Latency_REM = app.Latency_result.first_rem_minutes;
                latency_table = table(Segment_Beginning,Segment_End,Latency_NREM,Latency_REM);
                writetable(latency_table,filename,'Sheet','Latency (min)',"FileType","spreadsheet")                     % Latency
            end
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.UIFigure.Position = [150 50 828 685];
            ...
            % Create CalculateLatencyCheckBox
            app.CalculateLatencyCheckBox = uicheckbox(app.UIFigure);
            app.CalculateLatencyCheckBox.Text = 'Calculate Latency';
            app.CalculateLatencyCheckBox.Position = [694 115 117 23];
