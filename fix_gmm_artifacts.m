...
% Create a matrix regarding the non artifact epochs
non_artifact = true(length(x),1);
non_artifact(artifact.LFP_epoch) = false;
% Get the GMM prob. values and insert into the expected (whole) GMM
GMM_prob_all(non_artifact,:) = GMM.Prob.All(:,1:number_clusters);
...
% subtract_vector = (1:size(segments_timestamps,1))';
