function Visual_inspection= Visual_Inspection (recording_app_handle,LFP,EMG,x,y,label_y,output_path,continue_visual_inspection)
...
if num_ep_vis < 40  % 40 is the minimum number (if the number is lower, make it 40)
    num_ep_vis = 40;
end
...
%                 
        ...
        if size(Visual_inspection.AWAKE_idx,2)>=num_ep_vis + 1 && ...
                size(Visual_inspection.NREM_idx,2)>=num_ep_vis + 1 && ...
                size(Visual_inspection.REM_idx,2)>=num_ep_vis + 1 && ~app_handle.FinishreinspectionButton.Visible
            condition=0;
