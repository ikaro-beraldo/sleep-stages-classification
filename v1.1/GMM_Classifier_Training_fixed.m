...
function GMM_Classifier_Training_fixed(app,CA1_data,EMG_data,pre_process_state,load_data,selected_indices)
...
if load_data
    ...
    [LFP, EMG, outputPath] = pre_processing(app,CA1_data,EMG_data,pre_process_state);
...
if create_GMM % Create a GMM_Classification
    save(fullfile(outputPath,'GMM_Classification.mat'),'check_point_info')    % Create a new GMM file and save the check point
else
...
if (check_point_info.status && ~check_point_info.data_dist) || ~check_point_info.status
    ...
    set(gca,'Linewidth',figure_parameters.lw)
    set(gca,'Tickdir','out')
    title('EMG (z-score)','FontSize',figure_parameters.fontsize*1.2)
    ...
    yticklabels({'EMG (z-score)',label_y})
    xticks([1:size(figure_parameters.time_color,2)/(size(figure_parameters.time_vector,1)-1):size(figure_parameters.time_color,2) size(figure_parameters.time_color,2)])
...
[artifact,finished] = artifacts_detection(EMG,LFP,outputPath,app.Recording_app.ArtifactdetectionamplitudetresholdSDEditField.Value,...
    app.Recording_app.InferiorEditField.Value,app.Recording_app.SuperiorEditField.Value);
...
%     data_combined(:,2) = data_combined(:,2) + difference_theta_delta;    
    ...
    % Change the distribution to match the training data distribution
    difference_theta_delta = min(Training_data.LFP_used) - min(artifact.x_artifact_free);
    difference_emg = min(Training_data.EMG_used) - min(artifact.y_artifact_free);
...
if (check_point_info.status && ~check_point_info.visual_inspection) || ~check_point_info.status
    ...
    if app.Algorithm_params.run_visual_inspection
        Visual_inspection = Visual_Inspection (app.Recording_app,LFP,EMG,x,y,label_y,app.Output_path,app.Algorithm_params.continue_visual_inspection);
...
if (check_point_info.status && ~check_point_info.define_indx_thresh) || ~check_point_info.status
    ...
    plot_artifact_figures(GMM,LFP,x,y,artifact,outputPath,app.Recording_app.InferiorEditField.Value,app.Recording_app.SuperiorEditField.Value)
...
if (check_point_info.status && ~check_point_info.final_class) || ~check_point_info.status
    ...
    for jj=1:figure_parameters.figure_over_time
        ...
        yticklabels({'EMG (z-score)',label_y,'REM','NREM','AWAKE'})
        if figure_parameters.figure_over_time==2
            ...
            xlim([1 size(x,1)])
            xticks(1:figure_parameters.figure_over_time:figure_parameters.time_vector(end))
...
if (check_point_info.status && ~check_point_info.plot_comparison) || ~check_point_info.status
    ...
    set(gca,'Linewidth',figure_parameters.lw)
    set(gca,'Tickdir','out')
    title('Traning Model Data','FontSize',figure_parameters.fontsize*1.2)
...
if (check_point_info.status && ~check_point_info.final_freq) || ~check_point_info.status
    ...
    gscatter(x,y,aux_GMM.All_Sort,[figure_parameters.color.awake;figure_parameters.color.nrem;figure_parameters.color.rem],'.');
    legend off
    box off
    ylabel(label_y);
    xlabel('EMG (z-score)');
...
if (check_point_info.status && ~check_point_info.plot_representative) || ~check_point_info.status
    ...
    if app.Algorithm_params.plot_representative_epochs
        ...
        if ismember('LFP_epochs', listOfVariables) % Check if it has any of fields saved as variables
            DATA = load(file_path,'LFP_epochs','EMG_epochs');   % Works only if the DATA was save using -struct option
        else
        ...
        time_LFP=(1:1:size(DATA.LFP_epochs,2))./LFP.FS;
        if isfield(DATA,'EMG_processed_sampling_frequency')  % Check if the field FS exists
            time_EMG=(1:1:size(DATA.EMG_epochs,2))./DATA.EMG_processed_sampling_frequency;  % Uses the sampling frequency of the EMG
        else
            time_EMG=(1:1:size(DATA.EMG_epochs,2))./LFP.FS;  % Uses the sampling frequency of the LFP
        end
