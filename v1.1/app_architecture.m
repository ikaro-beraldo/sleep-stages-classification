...
for seg_idx = 1:params.n_segments
    ...
    if ~(isempty(W))  % Check if bouts matrix isn't empty
        W1=W;
        W(W(:,3)<TD.AWAKE,3)=NaN;
        
        aux=find((isnan(W(:,3)))==0);
        nbouts=size(aux,1);
        
        architecture.AWAKE.duration_all{seg_idx,1} = W.*params.epoch_length;
        architecture.AWAKE.duration_mean(seg_idx,1) = nanmean(W(:,3))*params.epoch_length;
...
%         architecture.AWAKE.total(seg_idx,1) = 0;
    ...
    if ~(isempty(W))  % Check if bouts matrix isn't empty
        ...
        architecture.NREM.duration_all{seg_idx,1} = W.*params.epoch_length;
        architecture.NREM.duration_mean(seg_idx,1) = nanmean(W(:,3))*params.epoch_length;
...
%         architecture.NREM.total(seg_idx,1) = 0;
    ...
    if ~(isempty(W))  % Check if bouts matrix isn't empty
        ...
        architecture.REM.duration_all{seg_idx,1} = W.*params.epoch_length;
        architecture.REM.duration_mean(seg_idx,1) = nanmean(W(:,3))*params.epoch_length;
