function [LFP, EMG, outputPath, pre_pro_params] = pre_processing_accel(app,CA1_data_import,accelX,accelY,accelZ,pre_processing_state,emg_accel_state)
...
pre_pro_params.notch = app.PowerLineNoiseHzEditField.Value:app.PowerLineNoiseHzEditField.Value:accelX.sampling_frequency/2;
pre_pro_params.notch_frequency_extension = 2;  % It means that the notch will consider a band [-2 <- notch_freq -> +2]
...
if pre_processing_state  % If it is true, the pre-processing step is going to be executed
    ...
    pre_pro_params.epoch_length = app.EpochLengthValue;   % Get it from the main window (RMS_pwelch_integrate)
    % Sampling frequency after decimate function
    switch app.Algorithm_preprocessing_step_final_sampling_frequency
        case 'Default'
            % EMG
            pre_pro_params.sampling_frequency_output = 250;   % Default values
            % LFP
    ...
    if strcmp(emg_accel_state,'Accel3')
        % Get the accelerometer derivative (add a zero value before, so it
        % has the same number of samples)
        accelX.data = [0 abs(diff(accelX.data))];
        ...
        clear accelX accelY accelZ
    else    % 1 channel (Only get the data from accelX and insert it into the default variable (accel_all)
        accel_all.data = accelX.data;
        accel_all.sampling_frequency = accelX.sampling_frequency;
        clear accelX
    end
    ...
    if mod(accel_all.sampling_frequency,pre_pro_params.sampling_frequency_output) ~= 0
        putative_sampling_frequencies = pre_pro_params.sampling_frequency_output:accel_all.sampling_frequency;    %Get the possible sampling frequencies
        % Get the sampling frequencies which can divide the input sampling
        % frequency
        putative_sampling_frequencies = putative_sampling_frequencies(mod(accel_all.sampling_frequency,putative_sampling_frequencies) == 0);
    ...
    %% 1st Step --> Divide the raw data in N seconds blocks (save it and then exclude)
    tic
    % Blocking Raw Data
    ...
    DATA.EMG_raw_sampling_frequency = accel_all.sampling_frequency;
    
    % Exclude the raw data
    clear CA1_data_import accel_all
    toc
    %% 3rd Step --> Decimate
    tic
    % LFP (CA1)
    ...
    n = 2; % filt order
    nyquist_rate = fs/2;
    if isempty(pre_pro_params.lowcutoff)    % If it is empty, execute a lowpass filter
        Wn = [pre_pro_params.highcutoff]/nyquist_rate;  % Lower and Upper frequency limits
        ftype = 'low'; % filter type
    else                                    % Else, execute a bandpass filter
...
%     toc
    ...
    if ~isfield(CA1_data_import,'epoch_length')
        ...
        switch selection
            ...
            case column
                ...
                accel_all.data = accel_all.data'; % Transpose
        ...
        accel_all.epoch_length = size(accel_all.data,2)/accel_all.sampling_frequency; % Insert the epoch length value
    end
    ...
    if isfield(accel_all,'filter_params')
        % Filter parameters
        pre_pro_params.highcutoff = accel_all.filter_params.high;    % High cutoff
        pre_pro_params.lowcutoff = accel_all.filter_params.low;  % Low cutoff
    end
    
    % Import EMG data
    RAW_DATA.EMG_raw_data = accel_all.data;
    
    % Save the EMG blocked raw data
    save(filenameLoad_raw_data,'-struct','RAW_DATA','-v7.3')
    clear RAW_DATA
    
    DATA.EMG_epochs = accel_all.data;
    DATA.EMG_processed_sampling_frequency = accel_all.sampling_frequency;
    % Saves EMG raw sampling frequency
    EMG.FS_raw =  accel_all.sampling_frequency;
    EMG.FS = accel_all.sampling_frequency;
...
for i=1:size(DATA.LFP_epochs,1)
    size_LFP_block = DATA.LFP_processed_sampling_frequency * pre_pro_params.epoch_length; % N s epoch
    ...
    for nn = 1:length(pre_pro_params.notch) % Loop for each noise frequency (main and harmonics)
        notch = (F >= pre_pro_params.notch(nn)-pre_pro_params.notch_frequency_extension & ...
            F <= pre_pro_params.notch(nn)+pre_pro_params.notch_frequency_extension);
    ...
    notch_freq = (Freq>=pre_pro_params.notch(1)-pre_pro_params.notch_frequency_extension & Freq<=pre_pro_params.notch(1)+pre_pro_params.notch_frequency_extension);
...
%% Define the unit of measurement accordingly to the average max value from each epoch
av_max_LFP = nanmean(max(DATA.LFP_epochs,[],2));
if abs(av_max_LFP - 0.5) < abs(av_max_LFP - 500) % It's in milivolts
    pre_pro_params.unit_measurement = 'mV';
elseif abs(av_max_LFP - 0.5) > abs(av_max_LFP - 500) % It's in microvolts
    pre_pro_params.unit_measurement = 'microV';
else % Volts
    pre_pro_params.unit_measurement = 'V';
end
...
clearvars -except LFP EMG DATA pre_pro_params filenameLoad_data_variables outputPath emg_accel_state
clc
...
time_info = datetime;
save(filenameLoad_data_variables,'LFP','emg_accel_state','pre_pro_params','time_info','-append')
